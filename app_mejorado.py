...
class ModelManager:
    ...
    def _cargar_modelo_imagen(self) -> Optional[CNNDetectionResNet]:
        ...
        try:
            if not os.path.exists(config.MODEL_IMAGE_PATH):
                logger.warning(f"⚠️ No se encontró el modelo en: {config.MODEL_IMAGE_PATH}")
                logger.warning("📥 Modo demostración activado para imágenes")
                return None
            
            modelo = CNNDetectionResNet()
            checkpoint = torch.load(config.MODEL_IMAGE_PATH, map_location=self.dispositivo)
    ...
    def _cargar_modelo_video(self) -> Optional[nn.Module]:
        ...
        try:
            modelo = timm.create_model(config.MODEL_VIDEO_NAME, pretrained=True, num_classes=2)
            modelo.to(self.dispositivo)
            modelo.eval()
            ...
            logger.error(f"❌ Error cargando modelo de video: {e}")
...
def analizar_imagen(imagen_input) -> str:
    ...
    try:
        with Timer() as timer:
            ...
            logger.info(f"✅ Análisis completado: {'FAKE' if es_fake else 'REAL'} ({probabilidad_fake:.2f}%)")
...
def analizar_video(video_path: str, progress=gr.Progress()) -> str:
    ...
    if video_path is None:
        return generar_reporte_error("No se proporcionó ningún video", "warning")
    ...
    if model_manager.modelo_video is None:
        return generar_reporte_error(
            "El modelo de análisis de video no está disponible. Por favor, verifica la instalación.",
            "error"
        )
    ...
    try:
        with Timer() as timer:
            ...
            if not cap.isOpened():
                return generar_reporte_error("No se pudo abrir el archivo de video", "error")
            
            # Obtener propiedades del video
            frames_totales = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            fps = cap.get(cv2.CAP_PROP_FPS)
            duracion = frames_totales / fps if fps > 0 else 0
            
            # Validar duración
            if duracion > config.MAX_VIDEO_DURATION_SECONDS:
                cap.release()
                return generar_reporte_error(
                    f"El video es demasiado largo ({duracion:.1f}s). Máximo: {config.MAX_VIDEO_DURATION_SECONDS}s",
                    "warning"
                )
            ...
            logger.info(f"📊 Analizando video: {frames_totales} frames, {duracion:.1f}s")
            ...
            if len(predicciones) < config.MIN_FACES_REQUIRED:
                return generar_reporte_error(
                    f"No se detectaron suficientes rostros para un análisis confiable (encontrados: {frames_analizados}). "
            ...
            logger.info(f"✅ Análisis completado: {'DEEPFAKE' if es_deepfake else 'REAL'} ({promedio_fake:.2f}%)")
        ...
        logger.error(f"❌ Error durante el análisis de video: {e}", exc_info=True)
        return generar_reporte_error(
